...
**The `MountHandle` is the seam for reaching that registry from outside the
tree.** `mount` succeeds with `{ registry }`; the testing harness reads
`ui.registry` off it. Before #199 the harness captured the service instead
(`Effect.flatMap(AtomRegistry, r => { captured = r; return app })` + a
...
- **No `unmount()`.** The ambient `Scope` owns teardown — closing it detaches
  the DOM, fires every finalizer, and disposes the registry. A second door
  would be a competing lifecycle with no way to keep them consistent.
- **No `sink`.** `RootSink` is provided AROUND `mount` (it is read from
  mount's context), so the caller installing it already holds it.
- **No `container`/root node.** The caller passed the element in.
